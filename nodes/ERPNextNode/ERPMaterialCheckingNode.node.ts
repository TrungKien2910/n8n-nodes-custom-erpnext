...
export class ERPMaterialCheckingNode implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const credentials = await this.getCredentials('erpnextCredentials');
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				...
				if (!workOrderName) {
					throw new Error('Work Order Name is required');
				}
...
export default ERPMaterialCheckingNode;
